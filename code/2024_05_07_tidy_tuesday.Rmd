...
tt_title_text    <- str_glue("Rolling Stones: Beatles Albums Ranked")
...
# The Beatles
``` {r}
# How has the ranking change for the Beatles' albums?
beatles_df <- rolling_stone_pivot_longer %>%
  filter(clean_name == "The Beatles") %>%
  select(clean_name, album, release_year, rank_year, rank)

number_of_beatles_albums <- length(unique(beatles_df$album))

beatles_df_status <- rolling_stone_pivot_longer %>%
  select(clean_name, album, genre, rank_year, rank)%>%
  mutate(rock_status = if_else(str_detect(str_to_lower(clean_name), "the beatles"), "The Beatles", "Not The Beatles")) %>%
  mutate(rock_status = as.factor(rock_status)) %>%
  mutate(rock_status = fct_relevel(rock_status, "The Beatles", "Not The Beatles"))


ggplot(beatles_df, aes(x = rank_year, y = rank, color = album)) + 
  geom_point() +
  geom_line() +
  labs(title = "Scatter Plot of Rank Over Years",
       x = "Year",
...
camcorder::gg_record(dir = here("outputs","2023-05-07"),
                     device = "png",
                     width = 8, height = 8,
                     units = "in", dpi = 320
)
...
beatles_df %>%
ggplot(aes(x = rank_year, y = rank, color = album)) +
...
# Subtitle
    ...
    axis.text.x = element_text(size=10,family = tt_family),
    axis.text.y = element_text(size=10,family = tt_family),
    axis.title.x = element_text(size=10,family = tt_family),
    axis.title.y = element_text(size=10,family = tt_family),
